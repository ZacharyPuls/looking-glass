...
final class SrOS extends Router {
    ...
    switch ($command) {
      case 'bgp':
        if (match_ipv6($parameter, false)) {
          $commands[] = 'show router bgp routes '.$parameter.' ipv6 hunt';
        } else if (match_ipv4($parameter, false)) {
          $commands[] = 'show router bgp routes '.$parameter.' ipv4 hunt';
        } else {
      ...
      case 'as-path-regex':
        if (match_aspath_regex($parameter)) {
          if (!$this->config['disable_ipv6']) {
            $commands[] = 'show router bgp routes ipv6 aspath-regex "'.$parameter.
              '"';
          }
          if (!$this->config['disable_ipv4']) {
            $commands[] = 'show router bgp routes ipv4 aspath-regex "'.$parameter.
              '"';
          }
        } else {
